...
const MovieDetails = () => {
  ...
  useEffect(() => {
    ...
    const getMovieDetails = async () => {
      try {
        const { data } = await axios.get(
          `https://api.themoviedb.org/3/movie/${id}?api_key=a2a53cbe6bd5d52a8a42446b0d093b87&append_to_response=videos`
        );
    ...
    const getMovieVideo = async () => {
      try {
        const { data } = await axios.get(
          `https://api.themoviedb.org/3/movie/${id}/videos?api_key=a2a53cbe6bd5d52a8a42446b0d093b87`
        );
    ...
    <>
      {loading ? (
        <div className="h-full text-white flex justify-center items-center">
          <span className="w-20 h-20 animate-pulse">
            <div className="w-11 h-11 bg-amber-500"></div>
          </span>
        </div>
      ) : (
        movieDetails.title && (
